...
public class LegManager : NetworkBehaviour
{
    ...
    void FootLookMouse()
    {
        ...
        //장애물 위에 있으면 접지된 지점을 따라가고, 아니면 마우스를 따라간다.
        if (isObstacle == true)
        {
        ...
        else
        {
            ...
            {
                foot.transform.position = mouseWorldPos;
            }
            else //최대거리 이상이면 방향만 따라가고 거리는 clamp
            {
                foot.transform.position = mouseWorldPos;
                Vector2 fp_dir = foot.transform.position - pelvis.transform.position;
                Vector2 clampedPosition = pelvis.transform.position + (Vector3)(fp_dir.normalized * 5);
